...
fn estimate_median() {
    ...
    let mut training = training::GradientDescentAl{ l0 : 0.9, t : 9.0, learned_events : 0.0 };
...
fn estimate_mean() {
    ...
    let mut training = training::GradientDescentAl{ l0 : 0.3, t : 4.0, learned_events : 0.0 };
...
fn linear_stochastic_gradient_descent() {
    ...
    let cost = cost::LeastSquares{};
    learn_history(&teacher, &cost, &mut model, history.iter().cycle().take(20).cloned());
...
fn logistic_sgd_2d_least_squares(){
    ...
    let mut model = model::Logistic{ linear: model::Linear{m : [0.0, 0.0], c : 0.0}};
